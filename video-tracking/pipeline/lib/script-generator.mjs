...
export async function generateScript(dest, { apiKey, outputDir, force = false }) {
  ...
  const prompt = `Write a 60-word narration script for a 30-second YouTube Short about ${dest.title}, Canada.
...
3. CTA (1 sentence) — direct viewers to discoveringcanada.info and subscribe
...
IMPORTANT: Write ALL numbers and dollar amounts as full words for text-to-speech. Say "thirty dollars" not "$30". Say "one hundred fifty dollars" not "$150". Say "discovering canada dot info" not "discoveringcanada.info".`;
