 ...
 *   2. Create project "Discover Canada"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Canada Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'canada'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Canada ${year}`,
      description: `${title}\n\nFull guide: https://discoveringcanada.info/${slug}/`,
      tags: [`Canada ${slug}`, 'Canada travel', `Canada ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Canada ✈️ #shorts`,
      description: `${title} — Full guide: https://discoveringcanada.info/destinations/${slug}/`,
      tags: [`${title} Canada`, 'Canada travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Canada — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Canada for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoveringcanada.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoveringcanada.info/companion/

#Canada #${title.replace(/\s+/g, '')} #CanadaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Canada`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER CANADA                 ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
