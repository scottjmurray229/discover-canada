...
const SAMPLE_PASTE = `Day 1 - Arrive Vancouver, check into downtown hotel
Day 2 - Stanley Park and Granville Island
Day 3 - Whistler day trip
Day 4 - Victoria via BC Ferries
Day 5 - Butchart Gardens
Day 6 - Banff flight
Day 7 - Lake Louise and Banff Ave`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Canada itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — BC, Rockies, Maritimes & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had no cell signal in Banff National Park and the app had everything — directions, restaurant picks, even local trail tips. Felt like traveling with a local friend."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day Canadian Rockies trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'bc') return t.route.match(/Vancouver|Victoria|Whistler|BC/i);
          if (filter === 'rockies') return t.route.match(/Banff|Jasper|Calgary|Lake Louise/i);
          if (filter === 'east') return t.route.match(/Toronto|Montreal|Quebec|Ottawa/i);
          return true;
        });
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'take the Translink SkyTrain to Waterfront, transfer to the Seabus' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'French & English phrases tuned to your destinations with pronunciation' },
            { icon: '📞', title: 'Key Contacts', desc: 'Emergency numbers, hospitals, trusted local drivers & guides for your specific route' },
            { icon: '🎒', title: 'Smart Packing', desc: 'Checklist auto-generated from your activities — alpine hiking adds microspikes' },
            { icon: '💰', title: 'Budget Tracker', desc: 'Log expenses, see typical costs, track in CAD or USD — all offline' },
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Canada situation, from taxi drivers to ski guides' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '📶', title: 'Works Offline', desc: 'Everything cached to your phone. No signal in the backcountry? No problem' },
          ].map((f, i) => (
            <div key={i} style={{
