...
function extractDestsFromText(text: string): string[] {
  ...
  // Keyword-based fallback: map common themes to popular Canada destinations
  const themeMap: Record<string, string[]> = {
    'ski': ['whistler', 'banff'],
    'skiing': ['whistler', 'banff'],
    'snowboard': ['whistler', 'banff'],
    'mountains': ['banff', 'jasper', 'whistler'],
    'rockies': ['banff', 'jasper', 'calgary'],
    'glacier': ['jasper', 'banff'],
    'lakes': ['banff', 'jasper'],
    'hiking': ['banff', 'jasper', 'whistler'],
    'wildlife': ['banff', 'jasper', 'victoria'],
    'whale': ['victoria', 'vancouver'],
    'aurora': ['jasper'],
    'northern lights': ['jasper'],
    'culture': ['montreal', 'toronto', 'quebec-city'],
    'history': ['quebec-city', 'ottawa'],
    'food': ['montreal', 'toronto', 'vancouver'],
    'french': ['montreal', 'quebec-city'],
    'museum': ['ottawa', 'toronto'],
    'city': ['toronto', 'vancouver', 'montreal'],
    'relax': ['victoria', 'prince-edward-island'],
    'beach': ['prince-edward-island', 'victoria'],
    'cycling': ['prince-edward-island', 'victoria'],
    'fall foliage': ['montreal', 'quebec-city'],
    'maple': ['montreal', 'ottawa'],
    'winter': ['whistler', 'banff', 'quebec-city'],
    'adventure': ['banff', 'jasper', 'whistler'],
    'family': ['niagara-falls', 'toronto', 'victoria'],
    'honeymoon': ['banff', 'victoria', 'whistler'],
    'romantic': ['banff', 'montreal', 'victoria'],
    'budget': ['ottawa', 'montreal', 'calgary'],
    'luxury': ['whistler', 'banff', 'vancouver'],
    'west': ['vancouver', 'whistler', 'victoria'],
    'east': ['montreal', 'quebec-city', 'prince-edward-island'],
  };
  ...
  // Ultimate fallback: popular starter destinations
  return ['banff', 'vancouver', 'toronto'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discovering Canada AI Trip Planner. You generate detailed day-by-day itineraries for Canada travel.

RULES:
- All prices in CAD (Canadian Dollars). Also show USD equivalent where helpful. Use rate: $1 USD = $1.36 CAD
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-banff"
- Note Parks Canada pass requirement ($10.50 CAD/day or $75.25 CAD/year) for national park destinations
- Canadian currency: use CAD$ prefix for clarity
...
{
  "title": "string — trip title like '7-Day Rocky Mountain Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · July 2026'",
  "totalBudget": { "php": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Banff'",
      "destination": "string — slug like 'banff'",
      "items": [
        {
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Canada trip:\n${parts.join('\n')}`;
