...
function buildAffiliateUrl(type: string | null | undefined, destination: string, description: string, tp?: TripParams): string {
  ...
  if (type === 'hotel') {
    ...
    const query = hotelName ? hotelName + ' ' + destName : destName + ' Canada';
    let url = `https://www.booking.com/searchresults.html?ss=${enc(query)}&aid=2778866&label=discoverphilippines`;
  ...
  if (type === 'tour') {
    ...
    return `https://www.klook.com/en-PH/search/result/?query=${enc(query)}&aid=112015&sub_id=discoverphilippines`;
  }
  if (type === 'transport') {
    return `https://www.klook.com/en-PH/search/result/?query=${enc(destName + ' transport transfer')}&aid=112015&sub_id=discoverphilippines`;
  }
  return `https://www.klook.com/en-PH/search/result/?query=${enc(destName)}&aid=112015&sub_id=discoverphilippines`;
}
...
function buildHotelsComUrl(destination: string, description: string): string {
  ...
  const landingPage = encodeURIComponent(`https://www.hotels.com/Hotel-Search?destination=${encodeURIComponent(query + ' Canada')}`);
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:linear-gradient(135deg,#0D7377,#14B8A6);border-radius:16px 16px 0 0;padding:28px 24px;text-align:center;">
      <h1 style="color:white;font-size:22px;margin:0 0 6px;">Your Canada Itinerary</h1>
    ...
    <div style="background:#F5F0E8;border-radius:0 0 16px 16px;padding:24px 20px;">
      ...
      <div style="text-align:center;padding:24px 0 16px;">
        <a href="https://discoveringcanada.info/plan/" style="display:inline-block;background:#E8654A;color:white;padding:14px 32px;border-radius:100px;text-decoration:none;font-weight:700;font-size:15px;">Generate Another Itinerary</a>
      </div>

      <p style="font-size:12px;color:#94A3B8;text-align:center;line-height:1.5;margin:0;">
        Generated by <a href="https://discoveringcanada.info/" style="color:#0D7377;text-decoration:none;">Discover Canada</a> AI Trip Planner<br>
    ...
    <p style="color:#94A3B8;font-size:11px;text-align:center;margin:16px 0 0;line-height:1.5;">
      <a href="https://discoveringcanada.info/legal/privacy/" style="color:#94A3B8;">Privacy Policy</a> &middot;
      <a href="https://discoveringcanada.info/legal/affiliate-disclosure/" style="color:#94A3B8;">Affiliate Disclosure</a>
    </p>
  </div>
</body>
</html>`;
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Discover Canada <hello@discoveringcanada.info>',
        to: [email],
        subject: `Your Canada Itinerary: ${it.title}`,
        html,
      }),
    });
    ...
    const setCookie = cookieSecret
      ...
      : 'dp_email=1; Path=/; Max-Age=2592000; SameSite=Lax';
